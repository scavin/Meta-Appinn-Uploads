...
(function () {
  ...
  const CONFIG = {
    ...
    /** 上传 API Token（可选，如果使用 Token 鉴权则填写） */
    API_TOKEN: '',
          ...
          onload: (response) => {
            ...
            try {
              ...
              const src = Array.isArray(data)
                ? data[0]?.src
                : data?.data?.[0]?.src ?? data?.src;
              if (!src) {
                return reject('无效的响应数据');
              }

              const fileUrl = src.startsWith('http')
                ? src
                : CONFIG.ASSETS_URL_PREFIX + src;
              resolve({
                url: fileUrl,
                filename,
              });
  ...
  /**
   * 构建鉴权 Header
   * @returns {Object} Header 字段
   */
  function buildAuthHeaders() {
    if (!CONFIG.API_TOKEN) {
      return {};
    }
    const token = CONFIG.API_TOKEN.startsWith('Bearer ')
      ? CONFIG.API_TOKEN
